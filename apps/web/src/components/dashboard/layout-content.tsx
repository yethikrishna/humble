	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				if (!sid) {
					const s = await createSessionRef.current.mutateAsync({ title: "Humble Onboarding" });
				...
				ob.setSessionId(sid);
				useTabStore.getState().openTab({ id: sid, title: "Humble Onboarding", type: "session", href: `/sessions/${sid}` });
			} catch (err) {
