...
> Humble (formerly Kortix / Suna) and [y0](https://github.com/yethikrishna/y0) ship as a single SaaS with two product tabs: the **y0** tab is the agent platform, the **Humble** tab is the autonomous-company OS. Each app links to the other in its top navigation, and both share the y0 brand assets.
...
We are building Humble as our own internal operating system at [Humble](https://github.com/yethikrishna/humble) to run our own companies and products. Every agent, skill, and workflow we ship is something we use ourselves — battle-tested against real workloads, not demos. 
...
```bash
curl -fsSL https://humble.yethikrishnar.pw/install | bash
```
...
Humble works best when it can run 24/7 — even when your laptop is closed. We recommend a server or VPS: [Humble Cloud](https://humble.yethikrishnar.pw/) (managed), your own server, or a VPS from [Hetzner](https://hetzner.com/) or [JustAVPS](https://justavps.com/).
...
kortix uninstall   Remove Humble completely
```
